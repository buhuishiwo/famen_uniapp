...
const filteredData = computed(() => {
  ...
  if (searchText.value) {
    ...
    result = result.filter(item => 
      ...
      (item.bodyMaterial || '').toLowerCase().includes(keyword) ||
      (item.gatePlateMaterial || '').toLowerCase().includes(keyword) ||
      (item.stemMaterial || '').toLowerCase().includes(keyword) ||
      (item.yokeMaterial || '').toLowerCase().includes(keyword)
    );
  }
